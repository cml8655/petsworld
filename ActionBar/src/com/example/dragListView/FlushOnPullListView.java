...
import com.pets.activity.NewDailyShareActivity;
import com.pets.util.CustomDataUtils;
 ...
 * 
 */
public class FlushOnPullListView extends ListView implements OnScrollListener,
		OnClickListener {
	// 拖拉ListView枚举所有状态
	...
	/**
	 * 常量接口，主要用于数据获取的key值定义
	 * 
	 * @author Administrator
	 * 
	 */
	private interface keys {
		public static final String CURR_UPDATE_TIME = "currentUpdateTime";
	}

	// 回到顶部view
	private PopupWindow backToTop;
	...
	private View headTools;// listview顶上的登录与发表按钮
	private Button login;// 登录按钮
	private Button newArticle;// 发表新说说按钮
	...
	public FlushOnPullListView(Context context) {
		super(context, null);
		initDragListView(context);
	}

	public FlushOnPullListView(Context context, AttributeSet attrs) {
		super(context, attrs);
		initDragListView(context);
	}
	...
	public void initDragListView(Context context) {
		...
		String time = CustomDataUtils.lastUpdateTime(keys.CURR_UPDATE_TIME,
				CustomDataUtils.DATE_LONG_FORMAT_YMDHMS);// 更新时间
	...
	public void initHeadView(Context context, String time) {
		...
		headTools = LayoutInflater.from(context).inflate(R.layout.head_tools,
				null);

		login = (Button) headTools.findViewById(R.id.list_head_login);
		newArticle = (Button) headTools.findViewById(R.id.list_head_new);

		login.setOnClickListener(this);
		newArticle.setOnClickListener(this);

		addHeaderView(headTools);
	...
	private void switchViewState(DListViewState state) {
		...
		case LV_PULL_REFRESH: {
			...
			// 开始设置时间上次刷新 时间
			this.setLastUpdateTime(CustomDataUtils.lastUpdateTime(
					keys.CURR_UPDATE_TIME,
					CustomDataUtils.DATE_LONG_FORMAT_YMDHMS));
	...
	private void backToTop(final AbsListView parent) {

		if (null == backToTop) {
			backToTop = new PopupWindow();
			backToTop.setWidth(LayoutParams.WRAP_CONTENT);
			backToTop.setHeight(LayoutParams.WRAP_CONTENT);
			...
			backBtn.setText("TOP");
			backBtn.setWidth(LayoutParams.MATCH_PARENT);
			backBtn.setHeight(LayoutParams.MATCH_PARENT);
			...
			backToTop.setContentView(layout);
		}

		if (!backToTop.isShowing())
			backToTop.showAtLocation(parent, Gravity.RIGHT | Gravity.BOTTOM,
					20, 20);
	}

	private void dismissTip() {
		if (null != backToTop && backToTop.isShowing()) {
			backToTop.dismiss();
		}
	}
